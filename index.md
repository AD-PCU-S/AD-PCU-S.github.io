...
|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|카메라|ㅁㅁㅁㅁ|플레이어의 카메라 좌표||
|포만감|ㅁㅁㅁㅁ|플레이어의 포만도 수치||
|이동|ㅁㅁㅁㅁ|키 입력을 받아 카메라가 보고있는 방향으로 전진||
|시야 제한|ㅁㅁㅁㅁ|일정이상 떨어진 오브젝트는 희미하게 보이게 함||

> [NPC]

|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|카메라|ㅁㅁㅁㅁ|플레이어와의 거리를 계산하여 상호작용이 가능하도록 활성화하고, 안내 생성||
...
|:---:|:---:|:---:|:---:|
|이동|ㅁㅁㅁㅁ|자동차 탑승시의 이동||
|연료|ㅁㅁㅁㅁ|자동차 이동에 필요한 연료. 수치가 0일때 이동 불가||
|거리|ㅁㅁㅁㅁ|플레이어와 거리를 계산하여 상호작용||
|회전|ㅁㅁㅁㅁ|플레이어의 컨트롤러 입력을 받아 방향값 추가||

> [도구(연료)]

|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|충전|ㅁㅁㅁㅁ|자동차에 이 오브젝트를 사용하면 연료 충전||

> [도구(식량)]

|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|섭취|ㅁㅁㅁㅁ|플레이어에 이 오브젝트를 사용하면 포만도 충전||
...
### 1. 핵심 규칙
- 개발자가 만든 루트를 따라 스토리를 진행한다.
- 최후의 엔딩에 도달한다.

### 2. 보조 규칙
- 배고픔을 채운다.
- 차의 기름을 채운다
- 위 2개가 부족해 질 때 주변 상황에 맞춰  이를 해결하며 생존한다.

## 7. 게임에서 사용될  공식

### 1. 오브젝트의 거리에 따라 보조창이 뜬다.

### 2. 캐릭터의 시야는 소규모 범위이다.

### 3. 캐릭터의 이동은 기본 걷기(km)를 유지하며, 차에 탑승시 차 이동소도는 (km)를 유지하고 에 따른다.

### 4. 캐릭터의 상호작용할 오브젝트의 구분은 플레이어의 거리에 따라 플레이어 중앙의 원과 이름으로 구분한다.
