...
|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|카메라|SteamVR의 SubCamera를 따라간다.|플레이어의 카메라 좌표||
|포만감|Player의 포만도는 최대 10으로 유지하되 Amount를 사용해서 플레이어의 UI에 노출되도록 한다.|플레이어의 포만도 수치||
|이동|Player의 Move는 5km/s으로 설정하며 좌측 Grip을 사용해 움직인다.|키 입력을 받아 카메라가 보고있는 방향으로 전진||
...
|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|이동|차의 이동은 10km/s로 하되 기어의 변속에 따라 20km/s까지 늘어난다. Player의 좌측 Grip을 통해 이동한다.|자동차 탑승시의 이동||
...
|속성|영문명칭|설명|비고|
|:---:|:---:|:---:|:---:|
|포만도|Hunger|Player는 일정 포만도를 가지며 시간에 비례해 낮아진다. Speed값에 영향을 주며, 0이 되면 Player의 이동속도는 0.5km/s로 조정한다.||
...
### 3. 캐릭터의 이동은 기본 걷기(5km/s)를 유지하며, 차에 탑승시 차 이동소도는 10km/s에서 20km/s까지 증가한다.
